...
CharArrayReader 是字符数组输入流。它和**[ByteArrayInputStream](http://www.cnblogs.com/skywang12345/p/io_02.html)**类似，只不过ByteArrayInputStream是字节数组输入流，而CharArray是字符数组输入流。CharArrayReader 是用于读取字符数组，它继承于Reader。操作的数据是以字符为单位！
